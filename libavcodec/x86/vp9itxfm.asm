...
%macro VP9_UNPACK_MULSUB_2D_4X 6 ; dst1 [src1], dst2 [src2], dst3, dst4, mul1, mul2 ; PIC
    punpckhwd          m%4, m%2, m%1
    punpcklwd          m%2, m%1
    PIC_BEGIN r4
    pmaddwd            m%3, m%4, [pic(pw_m%5_%6)]
    pmaddwd            m%4, [pic(pw_%6_%5)]
    pmaddwd            m%1, m%2, [pic(pw_m%5_%6)]
    pmaddwd            m%2, [pic(pw_%6_%5)]
    PIC_END
%endmacro
...
cglobal vp9_iwht_iwht_4x4_add, 3, 3, 0, dst, stride, block, eob
    ...
    pxor                m4, m4
    VP9_STORE_2X         0, 1, 5, 6, 4 ; dstq
    lea               dstq, [dstq+strideq*2]
    VP9_STORE_2X         2, 3, 5, 6, 4 ; dstq
...
%if cpuflag(ssse3)
    ...
    pmulhrsw            m0, m5
%else
    DEFINE_ARGS dst, stride, block, coef ; eobq->coefq
...
.idct2x2:
    ...
    movd                m1, [blockq+8]
    mova                m5, [pic(pw_11585x2)]
    mova                m6, [pic(pw_6270x2)]
    mova                m7, [pic(pw_15137x2)]
    VP9_IDCT4_2x2_1D
...
%if cpuflag(ssse3) ; if ssse3, PIC_BEGIN has already been performed
    PIC_CONTEXT_POP
%else
    %define rpicsave ; safe to push/pop rpic
    PIC_BEGIN r4
...
%macro VP9_IDCT8_1D_FINALIZE 0 ; blockq
...
%macro VP9_IDCT8_1D 0 ; blockq, PIC
%if ARCH_X86_64
    SWAP                 0, 8
    SWAP                 4, 9
%endif

    VP9_UNPACK_MULSUB_2W_4X 5,  3,  9102, 13623, D_8192_REG, 0, 4  ; m5=t5a, m3=t6a ; PIC
    VP9_UNPACK_MULSUB_2W_4X 1,  7, 16069,  3196, D_8192_REG, 0, 4  ; m1=t4a, m7=t7a ; PIC
...
%else
    VP9_UNPACK_MULSUB_2W_4X 7,  1, 11585, 11585, D_8192_REG, 0, 4 ; PIC
%endif
    VP9_UNPACK_MULSUB_2W_4X 2,  6, 15137,  6270, D_8192_REG, 0, 4  ; m2=t2a, m6=t3a ; PIC
...
%else
    ...
    VP9_UNPACK_MULSUB_2W_4X 0,  4, 11585, 11585, D_8192_REG, 5, 7 ; PIC
    UNSCRATCH            7, 9, blockq+64
%endif
    SUMSUB_BA            w,  6,  4, 5                       ; m6=t0a+t3a (t0), m4=t0a-t3a (t3)
    SUMSUB_BA            w,  2,  0, 5                       ; m2=t1a+t2a (t1), m0=t1a-t2a (t2)

    VP9_IDCT8_1D_FINALIZE ; blockq
%endmacro

%macro VP9_IDCT8_4x4_1D 0 ; blockq, PIC
    PIC_BEGIN r4
    pmulhrsw            m0, W_11585x2_REG                   ; m0=t1a/t0a
    pmulhrsw            m6, m2, [pic(pw_15137x2)]           ; m6=t3a
    pmulhrsw            m2, [pic(pw_6270x2)]                ; m2=t2a
    pmulhrsw            m7, m1, [pic(pw_16069x2)]           ; m7=t7a
    pmulhrsw            m1, [pic(pw_3196x2)]                ; m1=t4a
    pmulhrsw            m5, m3, [pic(pw_m9102x2)]           ; m5=t5a
    pmulhrsw            m3, [pic(pw_13623x2)]               ; m3=t6a
...
%macro VP9_IDCT8_2x2_1D 1 ; blockq, PIC
    PIC_BEGIN r4
...
%macro VP9_IDCT8_WRITEx2 6-8 5 ; line1, line2, tmp1, tmp2, zero, pw_1024/pw_16, shift ; dstq*
%if cpuflag(ssse3)
...
; - m8 holds m6 (SWAP)
; m6 holds zero
%macro VP9_IDCT8_WRITEOUT 0 ; dstq,strideq,blockq; PIC
%if ARCH_X86_64
%if cpuflag(ssse3)
    mova                m9, [pw_1024]
%else
...
%else
%if cpuflag(ssse3)
%define ROUND_REG [pic(pw_1024)]
%else
%define ROUND_REG [pic(pw_16)]
%endif
%endif
    ...
    PIC_BEGIN r4
    CHECK_REG_COLLISION "rpic","dstq","strideq","blockq"
    VP9_IDCT8_WRITEx2    0,  1, 5, 7, 6, ROUND_REG ; dstq
    lea               dstq, [dstq+2*strideq]
    VP9_IDCT8_WRITEx2    2,  3, 5, 7, 6, ROUND_REG ; dstq
    lea               dstq, [dstq+2*strideq]
    UNSCRATCH            5, 10, blockq+16
    UNSCRATCH            7, 11, blockq+32
    VP9_IDCT8_WRITEx2    4,  5, 0, 1, 6, ROUND_REG ; dstq
    lea               dstq, [dstq+2*strideq]
    UNSCRATCH            5, 8, blockq+ 0
    VP9_IDCT8_WRITEx2    5,  7, 0, 1, 6, ROUND_REG ; dstq
    PIC_END
...
%else
%define W_11585x2_REG [pic(pw_11585x2)]
%endif
...
%endif
    ...
    movd          [blockq], m4
%if cpuflag(ssse3)
    pmulhrsw            m0, [pic(pw_1024)]  ; (x*1024 + (1<<14))>>15 <=> (x+16)>>5
%endif
%rep 3
    VP9_STORE_2X         0,  0,  6,  7,  4 ; dstq
    lea               dstq, [dstq+2*strideq]
...
%else
%define W_3196x2_REG [pic(pw_3196x2)]
%define W_16069x2_REG [pic(pw_16069x2)]
%endif
    VP9_IDCT8_2x2_1D 1 ; PIC
    ...
    SWAP                 1, 4
    VP9_IDCT8_2x2_1D 2 ; PIC
%if ARCH_X86_64
    SWAP                 6, 8
%endif
    pxor                m6, m6  ; used for the block reset, and VP9_STORE_2X
    VP9_IDCT8_WRITEOUT ; dstq,strideq,blockq; PIC
%if ARCH_X86_64
    movd       [blockq+ 0], m6
    movd       [blockq+16], m6
%else
...
%else
%define D_8192_REG [pic(pd_8192)]
%endif
    VP9_IDCT8_1D ; blockq, PIC
%if ARCH_X86_64
...
%macro VP9_IADST8_1D 0 ; input/output=m0/1/2/3/4/5/6/7 ; PIC
%if ARCH_X86_64
    SWAP                     0, 8
    SWAP                     3, 9
    SWAP                     4, 10
...
%endif
    ...
    PIC_BEGIN r4
    CHECK_REG_COLLISION "rpic","blockq"
    VP9_UNPACK_MULSUB_2D_4X  5,  2,  0,  3, 14449,  7723    ; m5/2=t3[d], m2/4=t2[d] ; PIC
    VP9_UNPACK_MULSUB_2D_4X  1,  6,  4,  7,  4756, 15679    ; m1/4=t7[d], m6/7=t6[d] ; PIC
    ...
    VP9_UNPACK_MULSUB_2D_4X  7,  0,  1,  2, 16305,  1606    ; m7/1=t1[d], m0/2=t0[d] ; PIC
    VP9_UNPACK_MULSUB_2D_4X  3,  4,  5,  6, 10394, 12665    ; m3/5=t5[d], m4/6=t4[d] ; PIC
    ...
    VP9_UNPACK_MULSUB_2D_4X  0,  7,  1,  3, 15137,  6270    ; m0/1=t5[d], m7/3=t4[d] ; PIC
    VP9_UNPACK_MULSUB_2D_4X  5,  2,  4,  6,  6270, 15137    ; m5/4=t6[d], m2/6=t7[d] ; PIC
    SCRATCH                  1, 12, blockq+ 0*16
...
%else
    ...
    VP9_UNPACK_MULSUB_2W_4X  4, 3, 11585, 11585, D_8192_REG, 2, 5 ; PIC
    VP9_UNPACK_MULSUB_2W_4X  7, 0, 11585, 11585, D_8192_REG, 2, 5 ; PIC
...
%macro VP9_IDCT16_1D_START 6 ; src, nnzc, stride, scratch, scratch_stride, is_iadst ; PIC
    PIC_BEGIN r4
...
%if %2 <= 4
    ...
    pmulhrsw            m4, m3,  [pic(pw_16305x2)]  ; t14-15
    pmulhrsw            m3, [pic(pw_1606x2)]        ; t8-9
    pmulhrsw            m7, m0,  [pic(pw_m4756x2)]  ; t10-11
    pmulhrsw            m0, [pic(pw_15679x2)]       ; t12-13

    ; m8=t0, m9=t1, m10=t2, m11=t3, m12=t4, m14=t5, m13=t6, m15=t7
    ; m3=t8, m5=t9, m1=t10, m7=t11, m0=t12, m6=t13, m2=t14, m4=t15

    VP9_UNPACK_MULSUB_2W_4X 2, 5, 4, 3, 15137,  6270, [pic(pd_8192)], 1, 6 ; t9,  t14
    SCRATCH              4, 10, %4+ 1*%5
    SCRATCH              5, 11, %4+ 7*%5
    VP9_UNPACK_MULSUB_2W_4X 6, 1, 0, 7, 6270, m15137, [pic(pd_8192)], 4, 5 ; t10, t13
...
%else ; %2 > 4
    ...
    mova                m4, [%1+ 7*%3]      ; IN(7)
%if %2 <= 8
    pmulhrsw            m2, m5,  [pic(pw_16305x2)]  ; t15
    pmulhrsw            m5, [pic(pw_1606x2)]        ; t8
    pmulhrsw            m3, m4,  [pic(pw_m10394x2)] ; t9
    pmulhrsw            m4, [pic(pw_12665x2)]       ; t14
%else
    mova                m3, [%1+ 9*%3]      ; IN(9)
    ...
    VP9_UNPACK_MULSUB_2W_4X   5,   2, 16305,  1606, [pic(pd_8192)], 0, 1 ; t8,  t15
    VP9_UNPACK_MULSUB_2W_4X   3,   4, 10394, 12665, [pic(pd_8192)], 0, 1 ; t9,  t14
%endif

    SUMSUB_BA            w,  3,  5, 0       ; t8,  t9
    SUMSUB_BA            w,  4,  2, 0       ; t15, t14

    VP9_UNPACK_MULSUB_2W_4X   2,   5, 15137,  6270, [pic(pd_8192)], 0, 1 ; t9,  t14
    ...
    mova                m7, [%1+ 5*%3]      ; IN(5)
%if %2 <= 8
    pmulhrsw            m0, m7,  [pic(pw_14449x2)]  ; t13
    pmulhrsw            m7, [pic(pw_7723x2)]        ; t10
    pmulhrsw            m1, m6,  [pic(pw_m4756x2)]  ; t11
    pmulhrsw            m6, [pic(pw_15679x2)]       ; t12
%else
    mova                m0, [%1+11*%3]      ; IN(11)
    mova                m1, [%1+13*%3]      ; IN(13)

    VP9_UNPACK_MULSUB_2W_4X   7,   0, 14449,  7723, [pic(pd_8192)], 4, 5 ; t10, t13
    VP9_UNPACK_MULSUB_2W_4X   1,   6,  4756, 15679, [pic(pd_8192)], 4, 5 ; t11, t12
%endif
    ...
    VP9_UNPACK_MULSUB_2W_4X   6,   1, 6270, m15137, [pic(pd_8192)], 4, 5 ; t10, t13
...
%if cpuflag(ssse3) && %6 == 0
    SUMSUB_BA            w,  2,  5, 7
    SUMSUB_BA            w,  3,  4, 7
    pmulhrsw            m5, [pic(pw_11585x2)]    ; t10
    pmulhrsw            m4, [pic(pw_11585x2)]    ; t11
    pmulhrsw            m3, [pic(pw_11585x2)]    ; t12
    pmulhrsw            m2, [pic(pw_11585x2)]    ; t13
%else
    SCRATCH              6, 10, %4+ 1*%5
    VP9_UNPACK_MULSUB_2W_4X   5,   2, 11585, 11585, [pic(pd_8192)], 6, 7 ; t10, t13
    VP9_UNPACK_MULSUB_2W_4X   4,   3, 11585, 11585, [pic(pd_8192)], 6, 7 ; t11, t12
...
%if %2 <= 4
    ...
    pmulhrsw            m3, [pic(pw_11585x2)]     ; t0-t3
    pmulhrsw            m7, m4, [pic(pw_16069x2)] ; t6-7
    pmulhrsw            m4, [pic(pw_3196x2)]      ; t4-5

%if 0 ; overflows :(
    paddw               m6, m7, m4
    psubw               m5, m7, m4
    pmulhrsw            m5, [pic(pw_11585x2)]     ; t5
    pmulhrsw            m6, [pic(pw_11585x2)]     ; t6
%else
    VP9_UNPACK_MULSUB_2W_4X  5, 6, 7, 4, 11585, 11585, [pic(pd_8192)], 0, 1 ; t5,  t6
%endif
    PIC_END
...
%else ; %2 > 4
    ...
    mova                m7, [%1+ 6*%3]      ; IN(6)
%if %2 <= 8
    pmulhrsw            m0, m1,  [pic(pw_15137x2)]  ; t3
    pmulhrsw            m1, [pic(pw_6270x2)]        ; t2
    pmulhrsw            m5, m6, [pic(pw_16069x2)]   ; t7
    pmulhrsw            m6, [pic(pw_3196x2)]        ; t4
    pmulhrsw            m4, m7, [pic(pw_m9102x2)]   ; t5
    pmulhrsw            m7, [pic(pw_13623x2)]       ; t6
%else
    mova                m4, [%1+10*%3]      ; IN(10)
    mova                m0, [%1+12*%3]      ; IN(12)
    mova                m5, [%1+14*%3]      ; IN(14)

    VP9_UNPACK_MULSUB_2W_4X   1,   0, 15137,  6270, [pic(pd_8192)], 2, 3 ; t2,  t3
    VP9_UNPACK_MULSUB_2W_4X   6,   5, 16069,  3196, [pic(pd_8192)], 2, 3 ; t4,  t7
    VP9_UNPACK_MULSUB_2W_4X   4,   7,  9102, 13623, [pic(pd_8192)], 2, 3 ; t5,  t6
%endif
...
%if cpuflag(ssse3) && %6 == 0
    ...
    pmulhrsw            m6, [pic(pw_11585x2)]                          ; t6
%else
    VP9_UNPACK_MULSUB_2W_4X  5,  6, 11585, 11585, [pic(pd_8192)], 2, 3 ; t5,  t6
%endif

    SCRATCH              5, 15, %4+10*%5
    mova                m2, [%1+ 0*%3]        ; IN(0)
%if %2 <= 8
    pmulhrsw            m2, [pic(pw_11585x2)] ; t0 and t1
    PIC_END
    psubw               m3, m2, m0
    paddw               m0, m2
...
%if cpuflag(ssse3) && %6 == 0
    SUMSUB_BA            w,  3,  2, 5
    pmulhrsw            m3, [pic(pw_11585x2)] ; t0
    pmulhrsw            m2, [pic(pw_11585x2)] ; t1
    PIC_END
%else
    mova        [%1+ 0*%3], m0
    VP9_UNPACK_MULSUB_2W_4X  2,  3, 11585,  11585, [pic(pd_8192)], 5, 0 ; t0, t1
    PIC_END
    mova                m0, [%1+ 0*%3]
%endif
...
%macro VP9_IDCT16_1D 2-4 16, 1 ; src, pass, nnzc, is_iadst ; dstq*,strideq*,tmpq; PIC
%if %2 == 1
    PIC_BEGIN r4
    CHECK_REG_COLLISION "rpic",%1,,,"tmpq"
    VP9_IDCT16_1D_START %1, %3, 32, tmpq, 16, %4 ; tmpq, PIC
    PIC_END
...
%else ; %2 == 2
    PIC_BEGIN r4
    CHECK_REG_COLLISION "rpic",%1
    VP9_IDCT16_1D_START %1, %3, 32, %1, 32, %4 ; PIC

%if cpuflag(ssse3)
%define ROUND_REG [pic(pw_512)]
%else
%define ROUND_REG [pic(pw_32)]
%endif
...
%else ; !ARCH_X86_64
    ...
    VP9_IDCT8_WRITEx2    0,  1, 5, 6, 7, ROUND_REG, 6 ; dstq, PIC
    lea               dstq, [dstq+strideq*2]
    ...
    VP9_IDCT8_WRITEx2    4,  5, 1, 2, 0, ROUND_REG, 6 ; dstq, PIC
    PIC_END
    lea               dstq, [dstq+strideq*2]
%endif ; ARCH_X86_64 / !ARCH_X86_64
...
%macro VP9_STORE_2XFULL 6-7 strideq; dc, tmp1, tmp2, tmp3, tmp4, zero, stride ; dstq
    mova               m%3, [dstq]
    mova               m%5, [dstq+%7]
...
cglobal vp9_idct_idct_16x16_add, 4, 6, 16, 512, dst, stride, block, eob
    PIC_ALLOC "rpicsave" ; single PIC block, no need in lpiccache
    PIC_BEGIN r6 ; r6 not used in this function
    CHECK_REG_COLLISION "rpic","dstq","strideq","blockq","eobq"
...
%endif
    ...
    movd          [blockq], m5
%rep 7
    VP9_STORE_2XFULL    0, 1, 2, 3, 4, 5 ; dstq
    lea               dstq, [dstq+2*strideq]
%endrep
    ...
    CHECK_REG_COLLISION "rpic",,,,"cntq","dst_bakq","tmpq"
%if cpuflag(ssse3)
.idct8x8:
    mov               tmpq, rsp
    VP9_IDCT16_1D   blockq, 1, 8, 0 ; tmpq,blockq; PIC

    mov               cntd, 2
    mov           dst_bakq, dstq
.loop2_8x8:
    VP9_IDCT16_1D     tmpq, 2, 8, 0 ; dstq,strideq,tmpq; PIC
    lea               dstq, [dst_bakq+8]
    add               tmpq, 16
    dec               cntd
    jg .loop2_8x8
...
.loop1_full:
    VP9_IDCT16_1D   blockq, 1, 16, 0 ; tmpq,blockq; PIC
    add             blockq, 16
    add               tmpq, 256
    dec               cntd
    jg .loop1_full
    ...
    mov           dst_bakq, dstq
.loop2_full:
    VP9_IDCT16_1D     tmpq, 2, 16, 0 ; dstq,strideq,tmpq; PIC
    lea               dstq, [dst_bakq+8]
    add               tmpq, 16
    dec               cntd
    jg .loop2_full
...
%macro VP9_IDCT16_YMM_1D 0 ; PIC
    PIC_BEGIN r4
    VP9_UNPACK_MULSUB_2W_4X  1,  15, 16305,  1606, [pic(pd_8192)], 0, 4 ; t8,  t15
    VP9_UNPACK_MULSUB_2W_4X  9,   7, 10394, 12665, [pic(pd_8192)], 0, 4 ; t9,  t14

    SUMSUB_BA            w,  9,   1, 0      ; t8,  t9
    SUMSUB_BA            w,  7,  15, 0      ; t15, t14

    VP9_UNPACK_MULSUB_2W_4X 15,   1, 15137,  6270, [pic(pd_8192)], 0, 4 ; t9,  t14

    VP9_UNPACK_MULSUB_2W_4X  5,  11, 14449,  7723, [pic(pd_8192)], 0, 4 ; t10, t13
    VP9_UNPACK_MULSUB_2W_4X 13,   3,  4756, 15679, [pic(pd_8192)], 0, 4 ; t11, t12

    SUMSUB_BA            w,  5,  13, 0      ; t11, t10
    SUMSUB_BA            w, 11,   3, 0      ; t12, t13

    VP9_UNPACK_MULSUB_2W_4X  3,  13, 6270, m15137, [pic(pd_8192)], 0, 4 ; t10, t13
    ...
    SUMSUB_BA            w, 15,   1, 0
    SUMSUB_BA            w,  9,   7, 0
    pmulhrsw            m1, [pic(pw_11585x2)]    ; t10
    pmulhrsw            m7, [pic(pw_11585x2)]    ; t11
    pmulhrsw            m9, [pic(pw_11585x2)]    ; t12
    pmulhrsw           m15, [pic(pw_11585x2)]    ; t13

    ; even (tx8x8)
    mova                m4, [blockq+128]
    mova      [blockq+128], m5
    VP9_UNPACK_MULSUB_2W_4X   4,  12, 15137,  6270, [pic(pd_8192)], 0, 5 ; t2,  t3
    VP9_UNPACK_MULSUB_2W_4X   2,  14, 16069,  3196, [pic(pd_8192)], 0, 5 ; t4,  t7
    VP9_UNPACK_MULSUB_2W_4X  10,   6,  9102, 13623, [pic(pd_8192)], 0, 5 ; t5,  t6
    mova                m0, [blockq+  0]
    SUMSUB_BA            w,   8,   0, 5
    pmulhrsw            m8, [pic(pw_11585x2)]    ; t0
    pmulhrsw            m0, [pic(pw_11585x2)]    ; t1
    ...
    SUMSUB_BA            w,   2,  14, 5
    pmulhrsw           m14, [pic(pw_11585x2)]    ; t5
    pmulhrsw            m2, [pic(pw_11585x2)]    ; t6
    PIC_END
...
%macro VP9_STORE_YMM_DC_4X 6 ; reg, tmp1, tmp2, tmp3, tmp4, zero ; dstq,strideq
    mova              xm%2, [dstq]
...
%macro VP9_IADST16_1D 2 ; src, pass ; PIC
%assign %%str 16*%2
    mova                m0, [%1+ 0*32]  ; in0
    ...
    PIC_BEGIN r4
    CHECK_REG_COLLISION "rpic",%1,tmpq
    VP9_UNPACK_MULSUB_2D_4X  1,  0,  4,  5, 16364,   804       ; m1/4=t1[d], m0/5=t0[d] ; PIC
    VP9_UNPACK_MULSUB_2D_4X  2,  3,  7,  6, 11003, 12140       ; m2/7=t9[d], m3/6=t8[d] ; PIC
    SCRATCH              4, 8, tmpq+ 0*%%str
    VP9_RND_SH_SUMSUB_BA     3,  0,  6,  5,  4, [pic(pd_8192)] ; m3=t0[w], m0=t8[w]
    UNSCRATCH            4, 8, tmpq+ 0*%%str
    VP9_RND_SH_SUMSUB_BA     2,  1,  7,  4,  5, [pic(pd_8192)] ; m2=t1[w], m1=t9[w]
    ...
    VP9_UNPACK_MULSUB_2D_4X  0,  1,  6,  7, 15893,  3981       ; m0/6=t3[d], m1/7=t2[d] ; PIC
    VP9_UNPACK_MULSUB_2D_4X  3,  2,  4,  5,  8423, 14053       ; m3/4=t11[d], m2/5=t10[d] ; PIC
    SCRATCH              4, 12, tmpq+ 2*%%str
    VP9_RND_SH_SUMSUB_BA     2,  1,  5,  7,  4, [pic(pd_8192)] ; m2=t2[w], m1=t10[w]
    UNSCRATCH            4, 12, tmpq+ 2*%%str
    VP9_RND_SH_SUMSUB_BA     3,  0,  4,  6,  5, [pic(pd_8192)] ; m3=t3[w], m0=t11[w]
    ...
    VP9_UNPACK_MULSUB_2D_4X  3,  2,  7,  6, 14811,  7005       ; m3/7=t5[d], m2/6=t4[d] ; PIC
    VP9_UNPACK_MULSUB_2D_4X  0,  1,  4,  5,  5520, 15426       ; m0/4=t13[d], m1/5=t12[d] ; PIC
    SCRATCH              4, 9, tmpq+ 4*%%str
    VP9_RND_SH_SUMSUB_BA     1,  2,  5,  6,  4, [pic(pd_8192)] ; m1=t4[w], m2=t12[w]
    UNSCRATCH            4, 9, tmpq+ 4*%%str
    VP9_RND_SH_SUMSUB_BA     0,  3,  4,  7,  6, [pic(pd_8192)] ; m0=t5[w], m3=t13[w]
    ...
    VP9_UNPACK_MULSUB_2D_4X  0,  1,  6,  7, 16069,  3196       ; m1/7=t8[d], m0/6=t9[d] ; PIC
    VP9_UNPACK_MULSUB_2D_4X  3,  2,  5,  4,  3196, 16069       ; m3/5=t12[d], m2/4=t13[d] ; PIC
    SCRATCH              4, 9, tmpq+ 3*%%str
    VP9_RND_SH_SUMSUB_BA     3,  1,  5,  7,  4, [pic(pd_8192)] ; m3=t8[w], m1=t12[w]
    UNSCRATCH            4, 9, tmpq+ 3*%%str
    VP9_RND_SH_SUMSUB_BA     2,  0,  4,  6,  5, [pic(pd_8192)] ; m2=t9[w], m0=t13[w]
    ...
    VP9_UNPACK_MULSUB_2D_4X  3,  2,  7,  6, 13160,  9760       ; m3/7=t7[d], m2/6=t6[d] ; PIC
    VP9_UNPACK_MULSUB_2D_4X  0,  1,  4,  5,  2404, 16207       ; m0/4=t15[d], m1/5=t14[d] ; PIC
    SCRATCH              4, 9, tmpq+ 6*%%str
    VP9_RND_SH_SUMSUB_BA     1,  2,  5,  6,  4, [pic(pd_8192)] ; m1=t6[w], m2=t14[w]
    UNSCRATCH            4, 9, tmpq+ 6*%%str
    VP9_RND_SH_SUMSUB_BA     0,  3,  4,  7,  6, [pic(pd_8192)] ; m0=t7[w], m3=t15[w]
    ...
    VP9_UNPACK_MULSUB_2D_4X  5,  4,  6,  7,  9102, 13623       ; m5/6=t11[d], m4/7=t10[d] ; PIC
    VP9_UNPACK_MULSUB_2D_4X  3,  2,  1,  0, 13623,  9102       ; m3/1=t14[d], m2/0=t15[d] ; PIC
    SCRATCH              0, 9, tmpq+ 6*%%str
    VP9_RND_SH_SUMSUB_BA     3,  4,  1,  7,  0, [pic(pd_8192)] ; m3=t10[w], m4=t14[w]
    UNSCRATCH            0, 9, tmpq+ 6*%%str
    VP9_RND_SH_SUMSUB_BA     2,  5,  0,  6,  1, [pic(pd_8192)] ; m2=t11[w], m5=t15[w]
    ...
    SUMSUB_BA                w,  3,  7,  1
    PSIGNW                  m3, [pic(pw_m1)]                   ; m3=out1[w], m7=t10[w]
...
%if 0; cpuflag(ssse3)
    SUMSUB_BA                w,  7,  6,  1
    pmulhrsw                m7, [pic(pw_11585x2)]              ; m7=out6[w]
    pmulhrsw                m6, [pic(pw_11585x2)]              ; m6=out9[w]
%else
    VP9_UNPACK_MULSUB_2W_4X  6,  7, 11585, 11585, [pic(pd_8192)], 1, 0
%endif
    ...
    VP9_UNPACK_MULSUB_2D_4X  7,  6,  2,  3, 15137,  6270       ; m6/3=t13[d], m7/2=t12[d] ; PIC
    VP9_UNPACK_MULSUB_2D_4X  5,  4,  1,  0,  6270, 15137       ; m5/1=t14[d], m4/0=t15[d] ; PIC
    SCRATCH              0, 9, tmpq+ 2*%%str
    VP9_RND_SH_SUMSUB_BA     5,  6,  1,  3,  0, [pic(pd_8192)] ; m5=out2[w], m6=t14[w]
    UNSCRATCH            0, 9, tmpq+ 2*%%str
    VP9_RND_SH_SUMSUB_BA     4,  7,  0,  2,  1, [pic(pd_8192)]
    PSIGNW                  m4, [pic(pw_m1)]                   ; m4=out13[w], m7=t15[w]

    ; unfortunately, the code below overflows in some cases
%if 0; cpuflag(ssse3)
    SUMSUB_BA                w,  7,  6,  1
    pmulhrsw                m7, [pic(pw_m11585x2)]             ; m7=out5[w]
    pmulhrsw                m6, [pic(pw_11585x2)]              ; m6=out10[w]
%else
    PSIGNW                  m7, [pic(pw_m1)]
    VP9_UNPACK_MULSUB_2W_4X  7,  6, 11585, 11585, [pic(pd_8192)], 1, 0
%endif
    ...
    VP9_UNPACK_MULSUB_2D_4X  2,  3,  7,  5, 15137,  6270       ; m2/6=t5[d], m3/10=t4[d] ; PIC
    VP9_UNPACK_MULSUB_2D_4X  0,  4,  1,  6,  6270, 15137       ; m0/14=t6[d], m9/15=t7[d] ; PIC
    SCRATCH                  6, 10, tmpq+ 0*%%str
    VP9_RND_SH_SUMSUB_BA     0,  3,  1,  5,  6, [pic(pd_8192)]
    UNSCRATCH                6, 10, tmpq+ 0*%%str
    PSIGNW                  m0, [pic(pw_m1)]                   ; m0=out3[w], m3=t6[w]
    VP9_RND_SH_SUMSUB_BA     4,  2,  6,  7,  5, [pic(pd_8192)] ; m9=out12[w], m2=t7[w]
    ...
    SUMSUB_BA                w,  5,  7,  4
    PSIGNW                  m5, [pic(pw_m1)]                   ; m12=out15[w], m8=t3[w]

    ; unfortunately, the code below overflows in some cases, e.g.
    ; http://downloads.webmproject.org/test_data/libvpx/vp90-2-14-resize-fp-tiles-16-8-4-2-1.webm
%if 0 ; cpuflag(ssse3)
    SUMSUB_BA               w,   7,  6,  4
    pmulhrsw                m7, [pic(pw_m11585x2)]             ; m8=out7[w]
    pmulhrsw                m6, [pic(pw_11585x2)]              ; m1=out8[w]
    SWAP                     6,  7
    SUMSUB_BA                w,  3,  2,  4
    pmulhrsw                m3, [pic(pw_11585x2)]              ; m3=out4[w]
    pmulhrsw                m2, [pic(pw_11585x2)]              ; m2=out11[w]
%else
    SCRATCH                  5,  8, tmpq+10*%%str
    VP9_UNPACK_MULSUB_2W_4X  6,  7, 11585, m11585, [pic(pd_8192)],  5,  4
    VP9_UNPACK_MULSUB_2W_4X  2,  3, 11585, 11585, [pic(pd_8192)],  5,  4
...
%else ; !ARCH_X86_64
    ...
    mova       [tmpq+ 2*%%str], m7
    mova                    m2, [tmpq+ 9*%%str]
    VP9_IDCT8_WRITEx2        1,  2,  5,  7,  4, ROUND_REG, 6 ; PIC
    lea                   dstq, [dstq+strideq*2]
    mova                    m5, [tmpq+ 5*%%str]
    ...
    mova                    m0, [tmpq+13*%%str]
    mova                    m3, [tmpq+ 1*%%str]
    VP9_IDCT8_WRITEx2        0,  3,  1,  2,  4, ROUND_REG, 6 ; PIC
    PIC_END
%endif ; ARCH_X86_64 / !ARCH_X86_64
...
%macro VP9_IADST16_YMM_1D 0 ; blockq, PIC, amd64
    mova          [blockq+ 32], m3
    mova          [blockq+ 64], m7
    mova          [blockq+ 96], m8

    ; first half of round 1
    VP9_UNPACK_MULSUB_2D_4X  9,  6,  0,  3, 13160,  9760    ; m9/x=t7[d], m6/x=t6[d] ; PIC
    VP9_UNPACK_MULSUB_2D_4X  1, 14,  4,  7,  2404, 16207    ; m1/x=t15[d], m14/x=t14[d] ; PIC
    VP9_RND_SH_SUMSUB_BA    14,  6,  7,  3,  8, [pd_8192]   ; m14=t6[w], m6=t14[w]
    VP9_RND_SH_SUMSUB_BA     1,  9,  4,  0,  8, [pd_8192]   ; m1=t7[w], m9=t15[w]

    VP9_UNPACK_MULSUB_2D_4X 13,  2,  4,  7, 15893,  3981    ; m13/x=t3[d], m2/x=t2[d] ; PIC
    VP9_UNPACK_MULSUB_2D_4X  5, 10,  0,  3,  8423, 14053    ; m5/x=t11[d], m10/x=t10[d] ; PIC
    ...
    VP9_UNPACK_MULSUB_2D_4X  2, 13,  4,  7,  9102, 13623    ; m2/x=t11[d], m13/x=t10[d] ; PIC
    VP9_UNPACK_MULSUB_2D_4X  9,  6,  3,  0, 13623,  9102    ; m9/x=t14[d], m6/x=t15[d] ; PIC
    ...
    VP9_UNPACK_MULSUB_2D_4X 15,  0,  1,  9, 16364,   804    ; m15/x=t1[d], m0/x=t0[d] ; PIC
    VP9_UNPACK_MULSUB_2D_4X  7,  8, 10,  6, 11003, 12140    ; m7/x=t9[d], m8/x=t8[d] ; PIC
    VP9_RND_SH_SUMSUB_BA     8,  0,  6,  9, 14, [pd_8192]   ; m8=t0[w], m0=t8[w]
    VP9_RND_SH_SUMSUB_BA     7, 15, 10,  1, 14, [pd_8192]   ; m7=t1[w], m15=t9[w]

    VP9_UNPACK_MULSUB_2D_4X 11,  4, 10,  6, 14811,  7005    ; m11/x=t5[d], m4/x=t4[d] ; PIC
    VP9_UNPACK_MULSUB_2D_4X  3, 12,  1,  9,  5520, 15426    ; m3/x=t13[d], m12/x=t12[d] ; PIC
    ...
    VP9_UNPACK_MULSUB_2D_4X  0, 15,  6, 10, 16069,  3196    ; m15/x=t8[d], m0/x=t9[d] ; PIC
    VP9_UNPACK_MULSUB_2D_4X 11,  4,  9,  1,  3196, 16069    ; m11/x=t12[d], m4/x=t13[d] ; PIC
    ...
    ; round 3
    VP9_UNPACK_MULSUB_2D_4X 15,  0,  9, 12, 15137,  6270    ; m15/x=t13[d], m0/x=t12[d] ; PIC
    VP9_UNPACK_MULSUB_2D_4X  2, 13,  1,  6,  6270, 15137    ; m2/x=t14[d], m13/x=t15[d] ; PIC
    ...
    VP9_UNPACK_MULSUB_2D_4X  8,  7, 12,  9, 15137,  6270    ; m8/x=t5[d], m7/x=t4[d] ; PIC
    VP9_UNPACK_MULSUB_2D_4X  5, 10,  1,  6,  6270, 15137    ; m5/x=t6[d], m10/x=t7[d] ; PIC
...
%macro VP9_IDCT32_1D 2-3 32 ; src, pass, nnzc ; tmpq,dstq*,dst_endq*,stride2q*; PIC
%if %2 == 1
%assign %%str mmsize
%else
%assign %%str 64
%endif
    ...
    PIC_BEGIN r4
    CHECK_REG_COLLISION "rpic","tmpq"

    ; first do t0-15, this can be done identical to idct16x16
    VP9_IDCT16_1D_START %1, %3/2, 64*2, tmpq, 2*%%str, 1 ; PIC
...
%if %3 <= 8
    ...
    pmulhrsw             m1,  m4, [pic(pw_16364x2)] ; t31
    pmulhrsw             m4, [pic(pw_804x2)]        ; t16

    VP9_UNPACK_MULSUB_2W_4X   5,  0,  1,  4, 16069,  3196, [pic(pd_8192)], 6,  2 ; t17, t30

    pmulhrsw             m3,  m7, [pic(pw_m5520x2)] ; t19
    pmulhrsw             m7, [pic(pw_15426x2)]      ; t28

    SCRATCH               4, 13, tmpq+ 1*%%str
    SCRATCH               5, 12, tmpq+15*%%str

    ; TODO: check what m16069 is in disasm
    VP9_UNPACK_MULSUB_2W_4X   2,  6,  7,  3, 3196, m16069, [pic(pd_8192)], 4,  5 ; t18, t29
%else ; %3 > 8
    mova                 m0, [%1+ 1*64]
    mova                 m1, [%1+15*64]
%if %3 <= 16
    pmulhrsw             m5, m0, [pic(pw_16364x2)]
    pmulhrsw             m0, [pic(pw_804x2)]
    pmulhrsw             m4, m1, [pic(pw_m11003x2)]
    pmulhrsw             m1, [pic(pw_12140x2)]
%else
    mova                 m4, [%1+17*64]
    mova                 m5, [%1+31*64]

    VP9_UNPACK_MULSUB_2W_4X   0,  5, 16364,   804, [pic(pd_8192)], 2, 3 ; t16, t31
    VP9_UNPACK_MULSUB_2W_4X   4,  1, 11003, 12140, [pic(pd_8192)], 2, 3 ; t17, t30
%endif
    SUMSUB_BA             w,  4,  0,  2
    SUMSUB_BA             w,  1,  5,  2

    VP9_UNPACK_MULSUB_2W_4X   5,  0, 16069,  3196, [pic(pd_8192)], 2, 3 ; t17, t30
    ...
    mova                 m3, [%1+ 9*64]
%if %3 <= 16
    pmulhrsw             m7,  m3, [pic(pw_14811x2)]
    pmulhrsw             m3, [pic(pw_7005x2)]
    pmulhrsw             m6,  m2, [pic(pw_m5520x2)]
    pmulhrsw             m2, [pic(pw_15426x2)]
%else
    mova                 m7, [%1+23*64]
    mova                 m6, [%1+25*64]

    VP9_UNPACK_MULSUB_2W_4X   3,  7, 14811,  7005, [pic(pd_8192)], 4, 5 ; t18, t29
    VP9_UNPACK_MULSUB_2W_4X   6,  2,  5520, 15426, [pic(pd_8192)], 4, 5 ; t19, t28
%endif
    SUMSUB_BA             w,  3,  6,  4
    SUMSUB_BA             w,  7,  2,  4

    VP9_UNPACK_MULSUB_2W_4X   2,  6, 3196, m16069, [pic(pd_8192)], 4, 5 ; t18, t29
%endif ; %3 <= 8 / %3 > 8
    ...
    VP9_UNPACK_MULSUB_2W_4X   0,  5, 15137,  6270, [pic(pd_8192)], 6, 3 ; t18, t29
    VP9_UNPACK_MULSUB_2W_4X   1,  4, 15137,  6270, [pic(pd_8192)], 6, 3 ; t19, t28
...
%if %3 <= 8
    ...
    pmulhrsw             m5,  m0, [pic(pw_15893x2)] ; t27
    pmulhrsw             m0, [pic(pw_3981x2)]       ; t20

    VP9_UNPACK_MULSUB_2W_4X   1,  4,  5,  0,  9102, 13623, [pic(pd_8192)], 7,  2 ; t21, t26

    pmulhrsw             m6,  m3, [pic(pw_m2404x2)] ; t23
    pmulhrsw             m3, [pic(pw_16207x2)]      ; t24

    SCRATCH               5,  8, tmpq+ 5*%%str
    SCRATCH               4, 12, tmpq+11*%%str

    VP9_UNPACK_MULSUB_2W_4X   7,  2,  3,  6, 13623, m9102, [pic(pd_8192)], 4, 5 ; t22, t25
%else ; %3 > 8
    mova                 m4, [%1+ 5*64]
    mova                 m5, [%1+11*64]
%if %3 <= 16
    pmulhrsw             m1, m4, [pic(pw_15893x2)]
    pmulhrsw             m4, [pic(pw_3981x2)]
    pmulhrsw             m0, m5, [pic(pw_m8423x2)]
    pmulhrsw             m5, [pic(pw_14053x2)]
%else
    mova                 m0, [%1+21*64]
    mova                 m1, [%1+27*64]

    VP9_UNPACK_MULSUB_2W_4X   4,  1, 15893,  3981, [pic(pd_8192)], 2, 3 ; t20, t27
    VP9_UNPACK_MULSUB_2W_4X   0,  5,  8423, 14053, [pic(pd_8192)], 2, 3 ; t21, t26
%endif
    SUMSUB_BA             w,  0,  4,  2
    SUMSUB_BA             w,  5,  1,  2

    VP9_UNPACK_MULSUB_2W_4X   1,  4,  9102, 13623, [pic(pd_8192)], 2, 3 ; t21, t26
    ...
    mova                 m6, [%1+13*64]
%if %3 <= 16
    pmulhrsw             m3, m6, [pic(pw_13160x2)]
    pmulhrsw             m6, [pic(pw_9760x2)]
    pmulhrsw             m2, m7, [pic(pw_m2404x2)]
    pmulhrsw             m7, [pic(pw_16207x2)]
%else
    mova                 m2, [%1+29*64]
    mova                 m3, [%1+19*64]
    VP9_UNPACK_MULSUB_2W_4X   6,  3, 13160,  9760, [pic(pd_8192)], 4, 5 ; t22, t25
    VP9_UNPACK_MULSUB_2W_4X   2,  7,  2404, 16207, [pic(pd_8192)], 4, 5 ; t23, t24
%endif
    SUMSUB_BA             w,  6,  2,  4
    SUMSUB_BA             w,  3,  7,  4

    VP9_UNPACK_MULSUB_2W_4X   7,  2, 13623, m9102, [pic(pd_8192)], 4, 5 ; t22, t25
%endif ; %3 <= 8 / %3 > 8
    ...
    VP9_UNPACK_MULSUB_2W_4X   3,  6, 6270, m15137, [pic(pd_8192)], 4, 5 ; t20, t27
    VP9_UNPACK_MULSUB_2W_4X   2,  7, 6270, m15137, [pic(pd_8192)], 4, 5 ; t21, t26
...
%endif
    ...
    pmulhrsw             m7, [pic(pw_11585x2)]
    pmulhrsw             m4, [pic(pw_11585x2)]
    pmulhrsw             m1, [pic(pw_11585x2)]
    pmulhrsw             m5, [pic(pw_11585x2)]
...
%endif
    ...
    pmulhrsw             m3, [pic(pw_11585x2)]
    pmulhrsw             m7, [pic(pw_11585x2)]
    pmulhrsw             m2, [pic(pw_11585x2)]
    pmulhrsw             m1, [pic(pw_11585x2)]
%else ; !0 ; !cpuflag(ssse3)
    SCRATCH               0,  8, tmpq+15*%%str
    SCRATCH               6,  9, tmpq+17*%%str
    VP9_UNPACK_MULSUB_2W_4X  7,  4, 11585, 11585, [pic(pd_8192)], 0, 6
    mova    [tmpq+ 5*%%str], m7     ; t23
    UNSCRATCH             7, 10, tmpq+23*%%str
    VP9_UNPACK_MULSUB_2W_4X  1,  5, 11585, 11585, [pic(pd_8192)], 0, 6
    SCRATCH               1, 13, tmpq+25*%%str
    UNSCRATCH             1, 11, tmpq+27*%%str
    VP9_UNPACK_MULSUB_2W_4X  3,  7, 11585, 11585, [pic(pd_8192)], 0, 6
    VP9_UNPACK_MULSUB_2W_4X  2,  1, 11585, 11585, [pic(pd_8192)], 0, 6
...
%endif
    ...
    CHECK_REG_COLLISION "rpic","dstq","dst_endq"
%macro %%STORE_2X2 7-8 1 ; src[1-4], tmp[1-2], zero, inc_dst_ptrs ; dstq,dst_endq,stride2q*; PIC
    SUMSUB_BA            w, %4, %1, %5
    SUMSUB_BA            w, %3, %2, %5
    VP9_IDCT8_WRITEx2   %4, %3, %5, %6, %7, ROUND_REG, 6 ; dstq
%if %8 == 1
    CHECK_REG_COLLISION "rpic","stride2q"
    add               dstq, stride2q
%endif
...
%else ; !ARCH_X86_64
    ...
    mova                m2, [tmpq+ 0*%%str]
    mova                m3, [tmpq+ 4*%%str]
    %%STORE_2X2          2,  3,  0,  6, 4, 5, 1 ; dstq,dst_endq,stride2q; PIC
    ...
    mova                m6, [tmpq+29*%%str]
    mova                m0, [tmpq+21*%%str]
    %%STORE_2X2          2,  3,  0,  6, 4, 5, 1, 0 ; dstq,dst_endq; PIC
    PIC_END
%endif ; ARCH_X86_64 / !ARCH_X86_64
%undef ROUND_REG
%endif ; %2 == 1 / %2 != 1
...
cglobal vp9_idct_idct_32x32_add, 0, 6 + ARCH_X86_64 * 3, 16, 2048, dst, stride, block, eob
    PIC_ALLOC "rpicsave"
    PIC_BEGIN r6 ; r6 is not used in this function
    CHECK_REG_COLLISION "rpic","dstq","strideq","blockq","eobq",\
        "blockmp","dstmp","stridemp","eobm","[rsp]"
...
%endif
    ...
    movd          [blockq], m5
%rep 31
    VP9_STORE_2XFULL    0, 1, 2, 3, 4, 5, mmsize ; dstq
    add               dstq, strideq
%endrep
    VP9_STORE_2XFULL    0, 1, 2, 3, 4, 5, mmsize ; dstq
    PIC_CONTEXT_PUSH
...
%endif
    mov               tmpq, rsp
    VP9_IDCT32_1D   blockq, 1, 8 ; blockq,tmpq; PIC
...
.loop2_8x8:
    ...
    VP9_IDCT32_1D     tmpq, 2, 8 ; tmpq,dstq,dst_endq,stride2q; PIC
    add           dst_bakq, 8
    add               tmpq, 16
    dec               cntd
    jg .loop2_8x8
...
%endif
    ...
    mov               tmpq, rsp
.loop1_16x16:
    VP9_IDCT32_1D   blockq, 1, 16 ; blockq,tmpq; PIC
    add             blockq, 16
    add               tmpq, 512
    dec               cntd
    jg .loop1_16x16
...
.loop2_16x16:
    mov               dstq, dst_bakq
    lea           dst_endq, [dstq+stride30q]
    VP9_IDCT32_1D     tmpq, 2, 16 ; tmpq,dstq,dst_endq,stride2q; PIC
    add           dst_bakq, 8
    add               tmpq, 16
    dec               cntd
    jg .loop2_16x16
...
%endif
    ...
    mov               tmpq, rsp
.loop1_full:
    VP9_IDCT32_1D   blockq, 1 ; blockq,tmpq; PIC
    add             blockq, 16
    add               tmpq, 512
    dec               cntd
    jg .loop1_full
...
.loop2_full:
    mov               dstq, dst_bakq
    lea           dst_endq, [dstq+stride30q]
    VP9_IDCT32_1D     tmpq, 2 ; tmpq,dstq,dst_endq,stride2q; PIC
    add           dst_bakq, 8
    add               tmpq, 16
    dec               cntd
    jg .loop2_full
...
; for slightly improved interleaving, and it omits vpermq since the
; input is DC so all values are identical
%macro VP9_STORE_YMM_DC_2X2 6 ; reg, tmp1, tmp2, tmp3, tmp4, zero ; dstq,strideq
    mova               m%2, [dstq]
