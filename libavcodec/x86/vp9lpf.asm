...
%macro FILTER_INIT 9 ; tmp1, tmp2, cacheL, cacheH, dstp, stack_off, filterid, mask, source ; rp0*,1*,3*,4*,5*,rq0*,[rsp+]; PIC
    FILTER%7_INIT       %1, l, %3, %6 +      0 ; rp0*,1*,3*,4*,5*,rq0*,[rsp+]; PIC
    FILTER%7_INIT       %2, h, %4, %6 + mmsize ; rp0*,1*,3*,4*,5*,rq0*,[rsp+]; PIC
    packuswb            %1, %2
    MASK_APPLY          %1, %9, %8, %2
    mova                %5, %1
%endmacro
...
%macro FILTER6_INIT 4 ; %1=dst %2=h/l %3=cache, %4=stack_off ; rp0,1,3,rq0,[rsp+]; PIC
    CHECK_REG_COLLISION "rpic","rp0","rp1","rp3","rq0","[rsp+8*mmsize]"
    ...
    PIC_BEGIN r4
    paddw               %3, [pic(pw_4)]                 ; p3*3 + p2*2 + p1 + p0 + q0 + 4
    PIC_END
    psraw               %1, %3, 3                       ; (p3*3 + p2*2 + p1 + p0 + q0 + 4) >> 3
%endmacro

%macro FILTER14_INIT 4 ; %1=dst %2=h/l %3=cache, %4=stack_off ; rp4,5,[rsp+]; PIC
    CHECK_REG_COLLISION "rpic","rp4","rp5","[rsp+8*mmsize]"
    ...
    PIC_BEGIN r4
    paddw               %3, [pic(pw_8)]                 ; p7*7 + p6*2 + p5 + .. + p0 + q0 + 8
    PIC_END
...
; XXX-PQ
; P0:r04  P1:r01  P2:r01  P3:r0   P4:r02  P5:r02  P6:r05  P7:r02
; Q0:r01  Q1:r35  Q2:r32  Q3:r32  Q4:r3   Q5:r31  Q6:r31  Q7:r34
; dstq        strideq      mstrideq     dst2q    stride3q  mstride3q
; r0:P0-7,Q0  r1:P12,Q056  r2:P457,Q23  r3:Q1-7  r4:P0,Q7  r5:P6,Q1
...
; ..............AB -> AAAAAAAABBBBBBBB
%macro SPLATB_MIX 1-2 [pic(mask_mix)] ; PIC*
%if cpuflag(ssse3)
    pshufb     %1, %2
%else
    punpcklbw  %1, %1
...
%ifidn %1,h
    PIC_ALLOC "rpicsave" ; single PIC block will be generated
%else
    PIC_ALLOC ; scattered PIC blocks, need both rpicsave & lpiccache
%endif
...
%ifidn %1,h
    PIC_BEGIN dstq ; enclose scattered PIC blocks
%if mmsize==16
...
%endif
    ...
    ; In %1 == v expansions, P0..P7 & Q0..Q7 use 6 regs as base/index for
    ; addressing. To more easily see which regs are free in which runs of code,
    ; we 'mark' p0..p7,q0..q7 in vim this way (cf XXX-PQ:
    ; r0:P0-7,Q0  r1:P12,Q056  r2:P457,Q23  r3:Q1-7  r4:P0,Q7  r5:P6,Q1):
    ;   :1;/PIC-[A]/;/PIC-[Z]/:s/ ; XXX:r.*$//
    ;   :1;/PIC-[A]/;/PIC-[Z]/:s/^\([[:space:]]*[^[:space:]%;][^;]*\<r\?[pPqQ][0-7]\>.*\)$/\1 ; XXX:r/
    ;   :1;/PIC-[A]/;/PIC-[Z]/:s/^\([^;]*\<r\?\([pP][0-7]\|[qQ]0\)\>.*XXX:r.*\)$/\10/
    ;   :1;/PIC-[A]/;/PIC-[Z]/:s/^\([^;]*\<r\?\([pP][12]\|[qQ][056]\)\>.*XXX:r.*\)$/\11/
    ;   :1;/PIC-[A]/;/PIC-[Z]/:s/^\([^;]*\<r\?\([pP][457]\|[qQ][23]\)\>.*XXX:r.*\)$/\12/
    ;   :1;/PIC-[A]/;/PIC-[Z]/:s/^\([^;]*\<r\?\([qQ][1-7]\)\>.*XXX:r.*\)$/\13/
    ;   :1;/PIC-[A]/;/PIC-[Z]/:s/^\([^;]*\<r\?\([pP]0\|[qQ]7\)\>.*XXX:r.*\)$/\14/
    ;   :1;/PIC-[A]/;/PIC-[Z]/:s/^\([^;]*\<r\?\([pP]6\|[qQ]1\)\>.*XXX:r.*\)$/\15/

%if i386pic
%ifdef m8
    ; we intend to skip all %ifdef m8 branches, let's assert:
    %fatal "m8 defined in i386pic mode"
%endif
%endif

    ; PIC-A (start of 'marked' range)
    PIC_BEGIN r2
    CHECK_REG_COLLISION "rpic",,,,,,,,"Ed","Id","Hd","[rsp]"
...
%else ; %2 != 16 && mmsize != 8
%if cpuflag(ssse3)
    mova                m0, [pic(mask_mix)]
%endif
    movd                m2, Id
    movd                m3, Ed
    SPLATB_MIX          m2, m0
    SPLATB_MIX          m3, m0
%endif ; %2 == 16 || mmsize == 8  /  %2 != 16 && mmsize != 8
...
%endif ; ifdef m8 / !def m8
    ABSSUB_GT           m5, rp3, rp2, m2, m7, m0        ; m5 = abs(p3-p2) <= I ; XXX:r01
    ABSSUB_GT           m1, rp2, rp1, m2, m7, m0        ; m1 = abs(p2-p1) <= I ; XXX:r01
    por                 m5, m1
    ABSSUB_GT           m1, rp1, rp0, m2, m7, m0        ; m1 = abs(p1-p0) <= I ; XXX:r014
    por                 m5, m1
    ABSSUB_GT           m1, rq0, rq1, m2, m7, m0        ; m1 = abs(q1-q0) <= I ; XXX:r0135
    por                 m5, m1
    ABSSUB_GT           m1, rq1, rq2, m2, m7, m0        ; m1 = abs(q2-q1) <= I ; XXX:r235
    por                 m5, m1
    ABSSUB_GT           m1, rq2, rq3, m2, m7, m0        ; m1 = abs(q3-q2) <= I ; XXX:r23
    por                 m5, m1
    ABSSUB              m1, rp0, rq0, m7                ; abs(p0-q0) ; XXX:r014
    paddusb             m1, m1                          ; abs(p0-q0) * 2
    ABSSUB              m2, rp1, rq1, m7                ; abs(p1-q1) ; XXX:r0135
    PIC_BEGIN r2
    pand                m2, [pic(pb_fe)]                ; drop lsb so shift can work
...
%if %2 != 44 && %2 != 4
    mova                m6, [pic(pb_81)]                ; [1 1 1 1 ...] ^ 0x80
    ABSSUB_GT           m2, rp3, rp0, m6, m5            ; abs(p3 - p0) <= 1 ; PIC ; XXX:r04
%ifdef m8
    mova                m8, [pb_80]
%define rb80 m8
%else
%define rb80 [pic(pb_80)]
%endif
    ABSSUB_GT           m1, rp2, rp0, m6, m5, rb80      ; abs(p2 - p0) <= 1 ; PIC ; XXX:r014
    por                 m2, m1
    ABSSUB              m4, rp1, rp0, m5                ; abs(p1 - p0) ; XXX:r014
%if %2 <= 16
%if cpuflag(ssse3)
    pxor                m0, m0
%endif
...
%endif ; %2 <= 16 / %2 > 16
    ...
    por                 m2, m4                          ; (flat8in)
    ABSSUB              m4, rq1, rq0, m1                ; abs(q1 - q0) ; XXX:r0135
    pxor                m4, rb80                        ; PIC
    PIC_END ; r2
    PIC_BEGIN r4
    CHECK_REG_COLLISION "rpic",,,,,,,,"Ed","Id","Hd","[rsp]"
    ...
    por                 m2, m4                          ; (flat8in)
    ABSSUB_GT           m1, rq2, rq0, m6, m5, rb80      ; abs(q2 - q0) <= 1 ; PIC ; XXX:r0123
    por                 m2, m1
    ABSSUB_GT           m1, rq3, rq0, m6, m5, rb80      ; abs(q3 - q0) <= 1 ; PIC ; XXX:r0123
    %undef rb80
    por                 m2, m1                          ; flat8in final value
    pxor                m2, [pic(pb_ff)]
%if %2 == 84 || %2 == 48
    pand                m2, [pic(mask_mix%2)]
%endif
    PIC_END ; r4
%if %2 != 16
    PIC_BEGIN r2
...
%endif ; %2 == 44 / %2 == 4
    pxor                m7, m6
    ABSSUB              m4, rp1, rp0, m1                ; abs(p1 - p0) ; XXX:r014
    pxor                m4, m6
    pcmpgtb             m0, m4, m7                      ; abs(p1 - p0) > H (1/2 hev condition)
    ABSSUB              m4, rq1, rq0, m1                ; abs(q1 - q0) ; XXX:r0135
    pxor                m4, m6
...
%endif
    PIC_BEGIN r1
    ABSSUB_GT           m1, rp7, rp0, m6, m5            ; abs(p7 - p0) <= 1 ; PIC ; XXX:r024
    ABSSUB_GT           m7, rp6, rp0, m6, m5            ; abs(p6 - p0) <= 1 ; PIC ; XXX:r045
    por                 m1, m7
%ifdef m8
    mova                m8, [P5] ; XXX:r02
    mova                m9, [P4] ; XXX:r02
...
%define rp4 [P4]
%endif
    ABSSUB_GT           m7, rp5, rp0, m6, m5            ; abs(p5 - p0) <= 1 ; PIC ; XXX:r024
    por                 m1, m7
    ABSSUB_GT           m7, rp4, rp0, m6, m5            ; abs(p4 - p0) <= 1 ; PIC ; XXX:r024
    PIC_END ; r1
    PIC_BEGIN r2
    por                 m1, m7
%ifdef m8
...
%define rq5 [Q5]
%endif
    ABSSUB_GT           m7, rq4, rq0, m6, m5            ; abs(q4 - q0) <= 1 ; PIC ; XXX:r013
    por                 m1, m7
    ABSSUB_GT           m7, rq5, rq0, m6, m5            ; abs(q5 - q0) <= 1 ; PIC ; XXX:r013
    por                 m1, m7
%ifdef m8
    mova                m14, [Q6] ; XXX:r13
    mova                m15, [Q7] ; XXX:r34
...
%define rq7 [Q7]
%endif
    ABSSUB_GT           m7, rq6, rq0, m6, m5            ; abs(q4 - q0) <= 1 ; PIC ; XXX:r013
    por                 m1, m7
    ABSSUB_GT           m7, rq7, rq0, m6, m5            ; abs(q5 - q0) <= 1 ; PIC ; XXX:r0134
...
%endif ; %2 == 16
    ...
    ; filter2()
%if %2 != 44 && %2 != 4
    mova                m6, [pic(pb_80)]                ; already in m6 if 44_16
...
%endif
    ...
    pxor                m2, m6, rq0                     ; q0 ^ 0x80 ; XXX:r01
    pxor                m4, m6, rp0                     ; p0 ^ 0x80 ; XXX:r04
    psubsb              m2, m4                          ; (signed) q0 - p0
    pxor                m4, m6, rp1                     ; p1 ^ 0x80 ; XXX:r01
    pxor                m5, m6, rq1                     ; q1 ^ 0x80 ; XXX:r35
    psubsb              m4, m5                          ; (signed) p1 - q1
    paddsb              m4, m2                          ;   (q0 - p0) + (p1 - q1)
    paddsb              m4, m2                          ; 2*(q0 - p0) + (p1 - q1)
    paddsb              m4, m2                          ; 3*(q0 - p0) + (p1 - q1)
    paddsb              m6, m4, [pic(pb_4)]             ; m6: f1 = clip(f + 4, 127)
    paddsb              m4, [pic(pb_3)]                 ; m4: f2 = clip(f + 3, 127)
%ifdef m8
    mova                m14, [pb_10]                    ; will be reused in filter4()
%define rb10 m14
%else
%define rb10 [pic(pb_10)]
%endif
    SRSHIFT3B_2X        m6, m4, rb10, m7                ; f1 and f2 sign byte shift by 3 ; PIC
    SIGN_SUB            m7, rq0, m6, m5                 ; m7 = q0 - f1 ; XXX:r01
    SIGN_ADD            m1, rp0, m4, m5                 ; m1 = p0 + f2 ; XXX:r04
%if %2 != 44 && %2 != 4
%ifdef m8
...
%else
    pand                m6, m3, m0
%endif
    MASK_APPLY          m7, rq0, m6, m5                 ; m7 = filter2(q0) & mask / we write it in filter4() ; XXX:r01
    MASK_APPLY          m1, rp0, m6, m5                 ; m1 = filter2(p0) & mask / we write it in filter4() ; XXX:r04
    ...
    paddsb              m6, m2, [pic(pb_4)]             ; m6:  f1 = clip(f + 4, 127)
    paddsb              m2, [pic(pb_3)]                 ; m2: f2 = clip(f + 3, 127)
...
%else ; %2 in {44,4}
    pandn               m0, m3
%endif ; %2 !in {44,4} / %2 in {44,4}
    SIGN_SUB            m5, rq0, m6, m4                 ; q0 - f1 ; XXX:r01
    MASK_APPLY          m5, m7, m0, m4                  ; filter4(q0) & mask
    mova                [Q0], m5                        ; XXX:r01
    SIGN_ADD            m7, rp0, m2, m4                 ; p0 + f2 ; XXX:r04
    ...
    PIC_END ; r2, save/restore

    SIGN_ADD            m1, rp1, m6, m2                 ; p1 + f ; XXX:r01
    SIGN_SUB            m4, rq1, m6, m2                 ; q1 - f ; XXX:r35
    MASK_APPLY          m1, rp1, m0, m2                 ; m1 = filter4(p1) ; XXX:r01
    MASK_APPLY          m4, rq1, m0, m2                 ; m4 = filter4(q1) ; XXX:r35
    mova                [P1], m1 ; XXX:r01
    mova                [Q1], m4 ; XXX:r35
...
%endif
    mova                m1, [P2] ; XXX:r01
    PIC_BEGIN r5
    CHECK_REG_COLLISION "rpic","[P2]","rp0","rp1","rp3","rq0","[rsp+8*mmsize]" ; XXX:r014
    FILTER_INIT         m4, m5, m6, m7, [P2], %4, 6,             m3,  m1             ; [p2] ; rp0,1,3,rq0; PIC ; XXX:r014
    PIC_END
    mova                m1, [Q2] ; XXX:r23
    FILTER_UPDATE       m4, m5, m6, m7, [P1], %4, 0, 1, 2, 5, 3, m3,  "", rq1, "", 1 ; [p1] -p3 -p2 +p1 +q1 ; XXX:r0135
    FILTER_UPDATE       m4, m5, m6, m7, [P0], %4, 0, 2, 3, 6, 3, m3,  "", m1         ; [p0] -p3 -p1 +p0 +q2 ; XXX:r04
    FILTER_UPDATE       m4, m5, m6, m7, [Q0], %4, 0, 3, 4, 7, 3, m3,  "", rq3, "", 1 ; [q0] -p3 -p0 +q0 +q3 ; XXX:r0123
    FILTER_UPDATE       m4, m5, m6, m7, [Q1], %4, 1, 4, 5, 7, 3, m3,  ""             ; [q1] -p2 -q0 +q1 +q3 ; XXX:r35
    FILTER_UPDATE       m4, m5, m6, m7, [Q2], %4, 2, 5, 6, 7, 3, m3,  m1             ; [q2] -p1 -q1 +q2 +q3 ; XXX:r23
%endif ; %2 !in {44,4}
...
%endif ; ifdef m8 / !def m8
    PIC_BEGIN r4
    CHECK_REG_COLLISION "rpic","[P6]","rp4","rp5","[rsp+8*mmsize]" ; XXX:r025
    FILTER_INIT     m4, m5, m6, m7, [P6], %4, 14,                m1,  m3            ; [p6] ; p4,5,6; PIC ; XXX:r025
    PIC_END
    FILTER_UPDATE   m4, m5, m6, m7, [P5], %4,  8,  9, 10,  5, 4, m1, rp5s           ; [p5] -p7 -p6 +p5 +q1 ; XXX:r02
    FILTER_UPDATE   m4, m5, m6, m7, [P4], %4,  8, 10, 11,  6, 4, m1, rp4s           ; [p4] -p7 -p5 +p4 +q2 ; XXX:r02
    FILTER_UPDATE   m4, m5, m6, m7, [P3], %4,  8, 11,  0,  7, 4, m1, rp3s           ; [p3] -p7 -p4 +p3 +q3 ; XXX:r0
    FILTER_UPDATE   m4, m5, m6, m7, [P2], %4,  8,  0,  1, 12, 4, m1,  "", rq4, [Q4], 1 ; [p2] -p7 -p3 +p2 +q4 ; XXX:r013
    FILTER_UPDATE   m4, m5, m6, m7, [P1], %4,  8,  1,  2, 13, 4, m1,  "", rq5, [Q5], 1 ; [p1] -p7 -p2 +p1 +q5 ; XXX:r013
    FILTER_UPDATE   m4, m5, m6, m7, [P0], %4,  8,  2,  3, 14, 4, m1,  "", rq6, [Q6], 1 ; [p0] -p7 -p1 +p0 +q6 ; XXX:r0134
    FILTER_UPDATE   m4, m5, m6, m7, [Q0], %4,  8,  3,  4, 15, 4, m1,  "", rq7, [Q7], 1 ; [q0] -p7 -p0 +q0 +q7 ; XXX:r0134
    FILTER_UPDATE   m4, m5, m6, m7, [Q1], %4,  9,  4,  5, 15, 4, m1,  ""            ; [q1] -p6 -q0 +q1 +q7 ; XXX:r35
    FILTER_UPDATE   m4, m5, m6, m7, [Q2], %4, 10,  5,  6, 15, 4, m1,  ""            ; [q2] -p5 -q1 +q2 +q7 ; XXX:r23
    FILTER_UPDATE   m4, m5, m6, m7, [Q3], %4, 11,  6,  7, 15, 4, m1,  ""            ; [q3] -p4 -q2 +q3 +q7 ; XXX:r23
    FILTER_UPDATE   m4, m5, m6, m7, [Q4], %4,  0,  7, 12, 15, 4, m1, rq4s           ; [q4] -p3 -q3 +q4 +q7 ; XXX:r3
    FILTER_UPDATE   m4, m5, m6, m7, [Q5], %4,  1, 12, 13, 15, 4, m1, rq5s           ; [q5] -p2 -q4 +q5 +q7 ; XXX:r13
    FILTER_UPDATE   m4, m5, m6, m7, [Q6], %4,  2, 13, 14, 15, 4, m1, rq6s           ; [q6] -p1 -q5 +q6 +q7 ; XXX:r13
%endif ; if %2 == 16

    ; PIC-Z (end of 'marked' range)

%ifidn %1,h
    PIC_END ; dstq, save/restore ; enclosure for scattered PIC blocks
%endif
